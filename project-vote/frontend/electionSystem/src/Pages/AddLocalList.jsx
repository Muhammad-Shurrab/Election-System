...
function AddLocalList() {
  ...
  const handleAddLocalList = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 400 && error.response.data.error === 'A local listing with this name already exists.') {
  ...
  if (totalNumber >= 10) {
   alert('عدد أعضاء القائمة اكتمل');
    ...
    return;
  } else if (candidacyCourse === 'Muslim' && counts.Muslim >= 8) {
...
const getlocalListingInformation = async (localListingID) => {
  try {
    const response = await axios.get(`http://localhost:4026/api/LocalList/getlocalListingInformation/${localListingID}`);
...
const fetchLocalListingName = async (listingID) => {
    try {
      const response = await axios.get(`http://localhost:4026/api/LocalList/getLocalListing/${listingID}`);
...
const deleteCitizenLocaList = async (nationalID) => {
  ...
  try {
    const response = await axios.delete(`http://localhost:4026/api/LocalList/deleteCitizenLocaList/${nationalID}`);
    
    if (response.status === 200) {
      // عملية الحذف نجحت
      alert('تم حذف العضو بنجاح');
      // تحديث الحالة لإزالة العضو المحذوف من قائمة الأعضاء
      setMembers((prevMembers) => prevMembers.filter(member => member.nationalID !== nationalID));
      await getlocalListingInformation(localListingID);
...
const getCitizen = async (nationalID) => {
  try {
    const response = await axios.get(`http://localhost:4026/api/LocalList/getCitizen/${nationalID}`);
...
const updateCitizen = async (nationalID) => {
  try {
    ...
    await axios.put(`http://localhost:4026/api/LocalList/editCitizen/${nationalID}`);
    console.log(`Citizen with nationalID: ${nationalID} updated successfully`);
    
  } catch (error) {
    console.error(`Error updating citizen with nationalID: ${nationalID}`, error);
  }
};
        ...
        <div className="mb-8 overflow-hidden bg-white shadow-xl text-end">
          <div className="p-6 space-y-4">
            ...
            {!nameSubmitted ? (
              ...
              <div>
                <h3 className="mb-4 text-xl font-semibold text-center text-gray-700">اسم القائمة: {Name}</h3>
                <form onSubmit={handleAddlocalListingInformation}>
                  ...
                  <div className="flex justify-end gap-4 mt-2">
                    ...
                    <button type="button" onClick={handleCancelmember} className="inline-block bg-[#CE1126] text-white px-4 py-2 rounded-md text-sm font-semibold transition-colors duration-300 hover:bg-[#A50E1F]">
                      إلغاء
                    </button>
                  </div>
                </form> 
          ...
          {  totalNumber === 10 ?
            ...
            <button onClick={updateAllCitizens} className="inline-block bg-[#166534] text-white px-4 py-2 rounded-md text-sm font-semibold transition-colors duration-300 hover:bg-[#15803d] w-44">
                  اعتماد القائمة
            </button>
            </div> :
 ...
 <div className='flex flex-wrap justify-center gap-4 mt-12'>
          {members.length > 0 ? (
            members.map((member, index) => (
              <div key={index} className="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow w-80 dark:bg-gray-800 dark:border-gray-700">
                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                  {member.name || 'اسم غير متاح'}
                </h5>
                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
                  {member.nationalID || 'الرقم الوطني غير متاح'} :الرقم الوطني
                 </p>
                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
                  {member.email || 'الايميل غير متاح'}
                </p>
                <div className='flex justify-end gap-5 cursor-pointer'>
                  <MdDelete onClick={() => deleteCitizenLocaList(member.nationalID)} />
                </div>
