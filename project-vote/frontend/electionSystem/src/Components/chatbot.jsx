...
const ChatBot = () => {
    ...
    const findResponse = (query) => {
      ...
      const partialMatches = Object.keys(responses).filter((question) => {
        const cleanQuestion = question.replace(/[^\w\s]/gi, '').replace(/\s+/g, ' ').trim();
        return cleanQuestion.split(' ').some(word => 
          cleanQuery.split(' ').includes(word)
        );
      });
      ...
      const similarQuestions = Object.keys(responses).filter((question) => {
        const cleanQuestion = question.replace(/[^\w\s]/gi, '').replace(/\s+/g, ' ').trim();
        return cleanQuestion.toLowerCase().includes(cleanQuery.toLowerCase()) ||
      ...
      return { text: 'عذرًا، لم أفهم سؤالك. هل يمكنك إعادة صياغته؟', sender: 'bot' };
    };
    ...
    const handleFaqClick = (question) => {
      ...
      const botResponse = { text: responses[question] || "عذرًا، لم أجد إجابة لهذا السؤال.", sender: 'bot' };
  ...
  return (
    <div className="flex flex-col h-[66vh] w-1/3 mx-auto border border-gray-300 rounded-lg shadow-lg bg-gray-100">
      ...
      <div id="chat-container" className="flex-grow overflow-y-auto p-4 space-y-4">
