...
//     // الحصول على عدد الأصوات من جدول districts
...
//     // الحصول على عدد المواطنين من جدول citizens
//     const citizenCount = await Citizen.count({
//       where: { districtID}, // نفترض أن جدول citizens يحتوي على عمود districtID
//     });

//     if (citizenCount === 0) {
//       return res.status(400).json({ error: 'No citizens found in this district' });
//     }
...
const district = await districts.findOne({
    ...
    if (citizenCount === 0) {
      return res.status(400).json({ error: `No citizens found in district ${districtName}` });
    }
