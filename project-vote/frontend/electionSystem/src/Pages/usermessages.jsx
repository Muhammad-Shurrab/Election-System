...
const UserMessages = () => {
  ...
  const sendReply = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('/api/chat/admin-reply', {
        userId: selectedUser,
            ...
            {selectedUser ? (
              <>
                <h2 className="text-xl font-semibold mb-4 text-gray-700 flex items-center">
                  <FiMessageCircle className="mr-2 text-[#DA2A29]" /> محادثة مع {selectedUser}
                </h2>
                <div className="bg-gray-100 p-4 h-[calc(100vh-350px)] overflow-y-auto mb-4 rounded-md shadow-inner">
                  {userMessages[selectedUser].map((msg, index) => (
                    <motion.div
                      key={msg.id || index}
                      className={`mb-3 ${msg.is_admin ? 'text-right' : 'text-left'}`}
